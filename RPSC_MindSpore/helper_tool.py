...
class ConfigSemanticKITTI:
    ...
    test_batch_size = 40 # batch_size during test
...
class ConfigS3DIS:
    ...
    p_w = 0
    p_w_add = {i: 0 for i in range(max_epoch)}
    p_w_add[30] = 1
...
class ConfigSemantic3D:
    ...
    val_batch_size = 20  # batch_size during validation and test
    train_steps = 300  # Number of steps per epochs
    val_steps = 150  # Number of validation steps per epoch
...
class DataProcessing:
    ...
    def get_file_list(dataset_path, test_scan_num):
        seq_list = np.sort(os.listdir(dataset_path)) #[00,01,...,21]
    ...
    @staticmethod
    def data_augment(xyz, color, labels, idx, num_out):
        theta = 2 * 3.14592653 * np.random.rand()  # from 0 to 0.2pi
        ...
        # R3 = np.array([[np.cos(theta), 0, np.sin(theta)], [0, 1, 0], [-np.sin(theta), 0, np.cos(theta)]])

        ind_dup = np.where(labels != 13)[0]

        xyz_labeled = xyz[ind_dup, :]
        # noise = np.random.normal(loc=0, scale=0.01, size=xyz_labeled.shape)
        # xyz_noise = xyz_labeled + noise

        xyz_rotate = np.matmul(xyz, R)
        xyz_aug = np.concatenate([xyz, xyz_rotate], 0)
        # xyz_aug = np.concatenate([xyz, xyz_labeled], 0)
        color_labeled = color[ind_dup, :]
        # color_aug = np.concatenate([color, color_labeled, color_labeled], 0)
        # idx_aug = np.concatenate([idx, idx[ind_dup], idx[ind_dup]], 0)
        # labels_aug = np.concatenate([labels, labels[ind_dup], labels[ind_dup]], 0)
        color_aug = np.concatenate([color, color], 0)
        idx_aug = np.concatenate([idx, idx], 0)
        labels_aug = np.concatenate([labels, labels], 0)


        # xyz_aug = np.concatenate([xyz, np.matmul(xyz_labeled, R1), np.matmul(xyz_labeled, R2), np.matmul(xyz_labeled, R3)], 0)
        # color_labeled = color[ind_dup, :]
        # color_aug = np.concatenate([color, color_labeled, color_labeled, color_labeled], 0)
        # idx_aug = np.concatenate([idx, idx[ind_dup], idx[ind_dup], idx[ind_dup]], 0)
        # labels_aug = np.concatenate([labels, labels[ind_dup], labels[ind_dup], labels[ind_dup]], 0)

        idx = np.arange(len(xyz_aug))
        np.random.shuffle(idx)

        xyz_aug = xyz_aug[idx][:num_out, :]
        color_aug = color_aug[idx][:num_out, :]
        idx_aug = idx_aug[idx][:num_out]
        labels_aug = labels_aug[idx][:num_out]

        return xyz_aug, color_aug, idx_aug, labels_aug
