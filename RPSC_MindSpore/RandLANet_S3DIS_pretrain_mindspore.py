...
from main_S3DIS_pretrain_mindspore import S3DISDatasetGenerator, dataloader, ms_map
import argparse
import os
import numpy as np
...
class RandLAWithLoss(nn.Cell):
    ...
    def __init__(self, network):
        ...
        self.matrix_loss = get_matrix_loss()
    ...
    def construct(
        ...
        if current_epoch <= 10:
            ...
            w_sco = 1.0
        d = embedding.shape[1]
        valid_labels, valid_logits, valid_embedding = self.network.data_prep(
            logits, embedding, labels, d
        )
        label_loss = self.get_loss(valid_logits, valid_labels)

        score_loss = 1.0 * score_loss * w_sco

        matrix_loss = w_mat * self.matrix_loss(valid_embedding, valid_labels)

        loss = label_loss + matrix_loss + score_loss
...
class TrainingWrapper(nn.Cell):
    ...
    def construct(
        self, xyz, feature, neighbor_idx, sub_idx, interp_idx, labels, current_epoch
    ):
        ...
        loss = (
            self.network(
                xyz, feature, neighbor_idx, sub_idx, interp_idx, labels, current_epoch
            )
        )
        # opt update
        ...
        grads = self.grad(self.network, opt_weights)(
            xyz, feature, neighbor_idx, sub_idx, interp_idx, labels, current_epoch, sens
        )
...
if __name__ == "__main__":
    ...
    for i, data in enumerate(train_loader):
        ...
        # predict
        loss = network(xyz, features, neigh_idx, sub_idx, interp_idx, labels, i)
        print("i:", i, " loss:", loss)
