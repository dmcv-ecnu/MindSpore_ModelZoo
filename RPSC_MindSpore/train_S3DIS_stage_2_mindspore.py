...
from main_S3DIS_stage2_mindspore import dataloader, ms_map, S3DISDatasetGenerator
from helper_tool import ConfigS3DIS as cfg
from RandLANet_S3DIS_stage2_mindspore import (
    RandLANet,
    RandLAWithLoss,
    TrainingWrapper,
    get_param_groups,
...
def train(args, restore_snap):
    ...
    log_out("Model restored from " + restore_snap, log_file)
    ...
    # data loader
    dataset = S3DISDatasetGenerator(args.pseudo_label_path, args.gt_path)
    ...
    for epoch in range(first_epoch, cfg.iter_epoch + 1):
        log_out(f"=== EPOCH {epoch:d}/{cfg.iter_epoch:d} ===", log_file)
        network.set_train()
        ...
        for i, data in enumerate(train_loader):
            ...
            d = embedding.shape[1]
            valid_labels_ps, valid_logits_ps, _ = network.network_logits.data_prep(
                logits, embedding, labels, d
            )

            topk = P.TopK(sorted=True)

            _, predicted = topk(valid_logits_ps, 1)

            correct_prediction = P.Equal()(predicted.squeeze(1), valid_labels_ps)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--load_dir", type=str, default="ex", help="")
    ...
    parser.add_argument(
        "--gt_path", type=str, default="./S3DIS_gt", help="gt label path"
    )
    ...
    cfg.pseudo_label_dir = FLAGS.pseudo_label_path
    cfg.log_dir = FLAGS.log_dir
    cfg.total_log = FLAGS.total_log
    if FLAGS.knn:
        cfg.k_n = FLAGS.knn

    print(cfg.experiment_dir)
    cfg.saving = True
    snap_path = cfg.load_dir
