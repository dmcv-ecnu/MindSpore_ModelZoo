...
class S3DISDatasetGenerator:
    def __init__(
        self, pseudo_label_path, gt_save_path, data_type="ply", val_area="Area_5"
    ):
        ...
        # Save path of label points and their corresponding labels
        ...
        self.input_labels_gt = {"training": [], "validation": []}
        self.input_probs = {"training": [], "validation": []}
    ...
    def load_data(self, sub_grid_size):
        ...
        for _, file_path in enumerate(self.all_files):
            ...
            if self.val_area in cloud_name:
                ...
                ps_probs = np.zeros(
                    shape=(sub_labels_gt.shape[0], len(self.label_to_names) - 1)
                )
            else:
                pseudo_label_file = join(self.pseudo_label_path, cloud_name + ".npy")
                pseudo_labels = np.load(pseudo_label_file, allow_pickle=True).item()
                sub_labels = pseudo_labels["preds"]
                gt_label_file = join(self.gt_path, cloud_name + ".npy")
                gt_labels = np.squeeze(np.load(gt_label_file))
                sub_labels_gt = gt_labels
                all_select_label_indx = list(np.where(sub_labels_gt != 13)[0])
                ps_probs = pseudo_labels["probs"]
            ...
            self.input_labels_gt[cloud_split] += [sub_labels_gt]
            self.input_probs[cloud_split] += [ps_probs]
...
class ActiveLearningSampler(ds.Sampler):
    ...
    def spatially_regular_gen(self):
        ...
        for _ in range(self.n_samples * self.batch_size):  # num_per_epoch
            ...
            queried_pc_labels_gt = self.dataset.input_labels_gt[self.split][cloud_idx][
                queried_idx
            ]
            queried_pc_probs = self.dataset.input_probs[self.split][cloud_idx][
                queried_idx
            ]
            ...
            queried_pc_label = np.vstack((queried_pc_labels, queried_pc_labels_gt)).T
            queried_pc_label = np.concatenate(
                [queried_pc_label, queried_pc_probs], axis=-1
            )

            if len(points) < cfg.num_points:
                queried_pc_xyz, queried_pc_colors, queried_idx, queried_pc_label = (
                    DP.data_aug(
                        queried_pc_xyz,
                        queried_pc_colors,
                        queried_pc_label,
            ...
            queried_pc_labels = queried_pc_label[:, 0]
            queried_pc_labels_gt = queried_pc_label[:, 1]
            queried_pc_probs = queried_pc_label[:, 2:]

            queried_pc_probs = queried_pc_probs.astype(np.float32)
            queried_pc_labels_gt = queried_pc_labels_gt.astype(np.int32)
            ...
            yield queried_pc_xyz, queried_pc_colors, queried_pc_labels, queried_pc_labels_gt, queried_pc_probs, queried_idx, cloud_idx
...
def dataloader(dataset, **kwargs):
    ...
    return (
        ds.GeneratorDataset(
            train_sampler,
            ["xyz", "colors", "labels", "labels_gt", "probs", "q_idx", "c_idx"],
            **kwargs
        ),
        ds.GeneratorDataset(
            val_sampler,
