...
class RandLANet(nn.Cell):
    ...
    def construct(self, xyz, feature, neighbor_idx, sub_idx, interp_idx, labels):
        ...
        return scores.squeeze(-1), se_features_list
    @staticmethod
...
class Hloss(nn.LossBase):
    ...
    def construct(self, multihot_labels, supervised_features, training_epoch):
        ...
        if training_epoch < 5*500:
            self.lambda_nl = ms.Tensor(1., dtype = ms.float32)
            self.lambda_pl = ms.Tensor(0, dtype = ms.float32)
        elif training_epoch < 100*500 // 3:
            self.lambda_nl = ms.Tensor(1., dtype = ms.float32)
            self.lambda_pl = ms.Tensor(0, dtype = ms.float32)
        elif training_epoch < 100*500 // 3 * 2:
            self.lambda_nl = ms.Tensor(1., dtype = ms.float32)
            self.lambda_pl = ms.Tensor(0.5, dtype = ms.float32)
        else:
    ...
    def multi_hot_loss(self, labels, logits):
        ...
        valid_pl_logits = P.cast(valid_pl_logits, ms.float32)
        ...
        valid_nl_labels = P.cast(valid_nl_labels, ms.float32)
...
class RandLAWithLoss(nn.Cell):
    ...
    def construct(self, feature, labels, input_inds, cloud_inds, p0, p1, p2, p3, p4, n0, n1, n2, n3, n4, pl0, pl1, pl2,
        ...
        logits, se_features_list = self.network(xyz, feature, neighbor_idx,
