...
class S3DISDatasetGenerator:
    def __init__(self, dataset_dir, pseudo_label_path, gt_path, data_type='ply', val_area='Area_5'):
    ...
    def load_data(self):
        for _, file_path in enumerate(self.paths):
            ...
            if self.val_area in cloud_name:
                ...
                # 如果gt label对某个点标注了且跟pseudo label不同，置为gt label
                # sub_labels = gt_labels
...
def dataloader(ds_dir, args, **kwargs):
    dataset = S3DISDatasetGenerator(ds_dir, args.pseudo_label_path, args.gt_label_path, val_area=args.val_area)
