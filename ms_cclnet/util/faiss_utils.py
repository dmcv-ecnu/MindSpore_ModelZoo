...
def swig_ptr_from_FloatTensor(x):
    ...
    assert x.dtype == mindspore.float32
    return faiss.cast_integer_to_float_ptr(
        x.storage().data_ptr() + x.storage_offset() * 4)

def swig_ptr_from_LongTensor(x):
    assert x.is_contiguous()
    assert x.dtype == mindspore.int64, 'dtype=%s' % x.dtype
...
def search_index_pytorch(index, x, k, D=None, I=None):
    ...
    if D is None:
        D = Tensor((n, k), dtype=mindspore.float32)
    else:
        assert D.size() == (n, k)

    if I is None:
        I = Tensor((n, k), dtype=mindspore.int64)
    else:
        assert I.size() == (n, k)
...
def search_raw_array_pytorch(res, xb, xq, k, D=None, I=None,
    ...
    if D is None:
        D = Tensor((nq, k), dtype=mindspore.float32)
    else:
        assert D.shape == (nq, k)
        assert D.device == xb.device

    if I is None:
        I = Tensor((nq, k), dtype=mindspore.int64)
    else:
        assert I.shape == (nq, k)
