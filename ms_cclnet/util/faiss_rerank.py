...
def compute_jaccard_distance(target_features, k1=20, k2=6, print_flag=True, search_option=3, use_float16=False):
    ...
    #     index.add(target_features.cpu().numpy().astype('float32'))
    #     _, initial_rank = index.search(target_features.cpu().numpy().astype('float32'), k1)
    else:
        # CPU
        index = index_init_cpu(target_features.size(-1))
        index.add(target_features.cpu().numpy())
    ...
    for i in range(N):
        ...
        dist = 2-2*ops.mm(target_features[i].unsqueeze(0).contiguous(), target_features[k_reciprocal_expansion_index].t())
        if use_float16:
            V[i,k_reciprocal_expansion_index] = ops.softmax(-dist, axis=1).view(-1).cpu().numpy().astype(mat_type)
        else:
            V[i,k_reciprocal_expansion_index] = ops.softmax(-dist, axis=1).view(-1).cpu().numpy()
