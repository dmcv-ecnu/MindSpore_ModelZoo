...
class AttentionPool2d(nn.Cell):
    def __init__(self, spacial_dim: int, embed_dim: int, num_heads: int, output_dim: int = None):
        super().__init__()
        self.positional_embedding = Parameter(ops.randn(spacial_dim + 1, embed_dim, dtype=ms.float32) / embed_dim ** 0.5)
...
class VisionTransformer(nn.Cell):
    def __init__(self, h_resolution: int, w_resolution: int, patch_size: int, stride_size: int, width: int, layers: int, heads: int, output_dim: int):
        ...
        self.positional_embedding = Parameter(scale * ops.randn(h_resolution*w_resolution + 1, width))
        print("VisionTransformer", self.positional_embedding.shape)
...
def build_model(ckpt_dict: dict, h_resolution: int, w_resolution: int, vision_stride_size: int):
    ...
    if vit:
        ckpt_dict["visual.positional_embedding"] = resize_pos_embed(ckpt_dict["visual.positional_embedding"], model.visual.positional_embedding, h_resolution, w_resolution)
    else: #RN50
        ckpt_dict["visual.attnpool.positional_embedding"] = resize_pos_embed(ckpt_dict["visual.attnpool.positional_embedding"], model.visual.attnpool.positional_embedding, h_resolution, w_resolution)
...
def resize_pos_embed(posemb, posemb_new, hight, width):
    ...
    posemb = Parameter(ops.cat([posemb_token, posemb_grid.squeeze()], axis=0))
    return posemb