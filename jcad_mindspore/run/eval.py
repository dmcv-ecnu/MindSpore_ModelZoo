...
def evaluate(opt, args):
    ...
    pred_root = os.path.join(opt.Eval.pred_root)
    gt_root = os.path.join(opt.Eval.gt_root, 'masks')
    ...
    if args.verbose is True:
        samples = tqdm.tqdm(enumerate(zip(preds, gts)), desc=' - Evaluation', total=len(
    ...
    csv = os.path.join(opt.Eval.result_path, 'result_' + '.csv')
