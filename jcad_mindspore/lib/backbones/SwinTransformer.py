...
class SwinTransformerBlock(nn.Cell):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
                 ...
                 act_layer=act_layers["GELU"], norm_layer=nn.LayerNorm):
...
class BasicLayer(nn.Cell):
    ...
    def construct(self, x):
        """construct"""
        # x [B, C, H, W]
        # x = ops.permute(x, (0, 2, 3, 1)) # [B, H, W, C]
...
class PatchEmbed(nn.Cell):
    ...
    def __init__(self, image_size=384, patch_size=4, in_chans=3, embed_dim=96, norm_layer=None):
        super().__init__()
...
def SwinB():
    model = SwinTransformer(image_size=384, embed_dim=128, depths=[2, 2, 18, 2], num_heads=[4, 8, 16, 32], window_size=12)
    param_dict = mindspore.load_checkpoint("/home/tkk/YZZ/lib/backbones/swin384.ckpt")
