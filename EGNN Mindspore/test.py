...
class ModelTrainer(object):
    ...
    def validation(self, network):
        ...
        num_queries = self.arg.num_way * 1
        ...
        for iter in range(self.global_step + 1, self.arg.train_iteration + 1):
            ...
            if iter > 0 and iter % self.arg.print_every == 0 or iter == self.arg.train_iteration:
                ...
                with open('./log/log.txt', 'a') as f:
                    f.write('iter {} loss {}  train_acc {}  using {}s\n'.format(iter,
                                                                                        avg_loss['loss'],
                                                                                        avg_loss['acc'],
...
if __name__ == '__main__':
    ...
    if arg.pretrain:
        param_dict = load_checkpoint('./dataset/model_iter_85000_enc.ckpt')
        tmp_dict = {}
    ...
    gnn_module = GraphNetwork(in_features=arg.embed_size,
                              ...
                              dropout=arg.dropout)
    if arg.pretrain:
        param_dict = load_checkpoint('./dataset/model_iter_85000_gnn.ckpt')
        tmp_dict = {}
        for key in param_dict.keys():
            s = key[18:]
            if s[0] != '0':
