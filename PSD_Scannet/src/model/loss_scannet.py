...
class PSDWithLoss(nn.Cell):
    ...
    def construct(self, feature, aug_feature, labels, input_inds, cloud_inds,
        ...
        reducing_list = np.arange(self.num_classes)
        inserted_value = P.zeros((1,), ms.int32)  # [0]
        for ign_label in self.ignored_label_inds:
            reducing_list = np.concatenate((inserted_value, reducing_list[ign_label:]), axis=0)
        global_labels = P.gather(reducing_list, global_labels,0)
