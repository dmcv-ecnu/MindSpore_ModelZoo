...
from src.data.dataset_scannet import dataloader, ms_map
...
def run_eval(args):
    ...
    ckpts = sorted(ckpts, key=lambda ckpt: int(ckpt.stem.split('_')[0].split('-')[1]),reverse=True)
    best_miou = 0.0
    print(ckpts[0])
    best_ckpt = ckpts[0]
    best_names = []
    ...
    while last_min < num_votes:
        ...
        if last_min + 1 < new_min:
            ...
            for i_test in range(num_val):
                ...
                preds = np.array(idlist)[preds].astype(np.int32)
                ...
                # Confs
                confusion_list += [confusion_matrix(labels_valid, pred_valid, idlist)]
            ...
            if int(np.ceil(new_min)) % 1 == 0:
                ...
                for i_test in range(num_val):
                    ...
                    ########
                    preds = np.argmax(proj_probs_list[i_test], axis=1)
                    preds = np.array(idlist)[preds].astype(np.uint8)
                    ...
                    acc = np.sum(pred_valid == labels_valid) / len(labels_valid)
                    logger.info(dataset.input_names['validation'][i_test] + ' Acc:' + str(acc))

                    confusion_list += [confusion_matrix(y_true=labels_valid, y_pred=pred_valid, labels=idlist)]
