...
class LrHrImages(FolderImagePair):
    def __init__(self, lr_pattern:str, hr_pattern, reader=None):
    ...
    def __getitem__(self, idx):
        _, *images = super(LrHrImages, self).__getitem__(idx) # 第一个元素pair_key赋值给匿名变量丢弃，剩余部分全部赋值给images，images变成数组（列表）
        return tuple(images) # 根据datasetgenerator类的需求将其转换成元组
...
def create_dataset_DIV2K(args, dataset_type="train", num_parallel_workers=10, shuffle=True):
    ...
    column_names = []
    lrs_pattern = []
    for lr_sacle in multi_lr_scale:        
...
def create_dataset_DIV2K_test(args, dataset_type="valid", num_parallel_workers=10, shuffle=True):
    ...
    # get HR_PATH/*.png
    dir_div2k = os.path.join(dataset_path, "DIV2K")
    dir_hr = os.path.join(dir_div2k, f"DIV2K_{dataset_type}_HR") # 高分图片，包括train和valid
    hr_pattern = os.path.join(dir_hr, "*.png")
    ...
    generator_dataset = generator_dataset.batch(1, drop_remainder=False) # batch为1是为了统一Tensor的维度
